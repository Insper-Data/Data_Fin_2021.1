...
words = ["petr4", " ibovespa", "divida", "cor", "açoes", "restaurante", "portifolio", "inflaçao",
...
retorno_1F_petr4 = list(range(len(words)))
retorno_1F_petr4 = pd.DataFrame(retorno_1F_petr4)
retorno_1F_petr4.set_index(words[0], inplace = True)

sharpe_1F_petr4 = list(range(len(words)))
sharpe_1F_petr4 = pd.DataFrame(sharpe_1F_petr4)
sharpe_1F_petr4.set_index(words[0], inplace = True)
...
for i in range(,len(words)):
    ...
    retorno_1F_petr4.iloc[i,0] = ganhoestrat[-1]
    
    sharpe_1F_petr4.iloc[i,0] = bf.sharpe_aa(ganhoestrat)